...
export async function getTransactions(query: TransactionQuery = {}) {
  return apiRequest<PaginatedResponse<ApiTransaction>>(`/transactions${queryString(query)}`);
}

export async function getTransactionById(id: string) {
  return apiRequest<ApiTransaction>(`/transactions/${encodeURIComponent(id)}`);
}

export async function createTransaction(payload: Record<string, unknown>) {
  return apiRequest<ApiTransaction>("/transactions", { method: "POST", body: JSON.stringify(payload) });
}

export async function updateTransaction(id: string, payload: Record<string, unknown>) {
  return apiRequest<ApiTransaction>(`/transactions/${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify(payload) });
}
