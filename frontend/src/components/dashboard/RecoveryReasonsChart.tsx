...
interface RecoveryReasonsChartProps {
  data: Array<{ name: string; value: number }>;
}

export function RecoveryReasonsChart({ data }: RecoveryReasonsChartProps) {
  return (
    <Card title="Top recovery reasons" className="h-[290px]">
      <div className="h-[230px] w-full">
        <ResponsiveContainer>
          <BarChart data={data} margin={{ top: 8, right: 12, left: -20, bottom: 0 }}>
            <CartesianGrid stroke="rgba(255,255,255,0.06)" vertical={false} />
            <XAxis dataKey="name" tickLine={false} axisLine={false} stroke="#A69D90" fontSize={11} />
            ...
            <Tooltip
              formatter={(value) => {
                ...
                return [`${numeric}%`, "Share"] as [string, string];
              }}
